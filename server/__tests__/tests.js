...
test("GET /players no pagination", async () => {
  ...
  await supertest(app).get("/players?page=1")
      .expect(200)
      .then((response) => {
      expect(response.body.results.length).toEqual(10)
    });
  await supertest(app).get("/players?page=1&pagesize=20")
      .expect(200)
      .then((response) => {
...
test("GET /matches no pagination", async () => {
  ...
  await supertest(app).get("/matches/D1?page=1")
    .expect(200)
    .then((response) => {
      expect(response.body.results.length).toEqual(10)
    });
  await supertest(app).get("/matches/D1?page=1&pagesize=20")
    .expect(200)
    .then((response) => {
      expect(response.body.results.length).toEqual(20)
    });
...
test("GET /match basic", async () => {
  ...
  await supertest(app).get("/match?id=1241341341")
      .expect(200)
      .then((response) => {
...
test("GET /search/matches basic 1", async () => {
  ...
  await supertest(app).get("/search/matches?Home=erafay&Away=Haafea")
    .expect(200)
    .then((response) => {
      expect(response.body.results.length).toEqual(0)
    });

  await supertest(app).get("/search/matches?Home=Ba&Away=ertha")
    .expect(200)
    .then((response) => {
      expect(response.body.results.length).toEqual(2)
    });

  await supertest(app).get("/search/matches?Home=Ba&Away=ertha&page=1")
    .expect(200)
    .then((response) => {
      expect(response.body.results.length).toEqual(2)
    });
...
test("GET /search/players basic 1", async () => {
  await supertest(app).get("/search/players?Name=Pepe&Nationality=Spain&Club=Lazio&page=1")
    .expect(200)
    .then((response) => {
      expect(response.body.results.length).toEqual(1)
